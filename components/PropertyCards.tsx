            ...
            <div className="mt-3 grid grid-cols-2 gap-2 sm:mt-4 sm:gap-2.5">
              {details.map((detail) => {
                ...
                return (
                  <div
                    key={detail.label}
                    className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/8 px-2.5 py-2 sm:gap-3 sm:rounded-[0.9rem] sm:px-3 sm:py-2.5"
                  >
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[rgba(212,175,55,0.18)] text-(--color-gold-400) sm:h-9 sm:w-9">
                      <Icon size={13} aria-hidden="true" className="sm:size-3.75" />
                    </span>
                    <div className="min-w-0">
                      <p className="line-clamp-1 text-xs font-semibold text-white sm:text-sm">{detail.value}</p>
                      <p className="mt-0.5 line-clamp-1 text-[0.55rem] font-semibold uppercase tracking-widest text-white/50 sm:text-[0.65rem]">{detail.label}</p>
                    </div>
                  </div>
                );
              })}
